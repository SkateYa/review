...
1。更好的逻辑复用与代码组织 (composition组合式api)

optionSAPI(1日) => compositionAPI（新），效果：代码组织更方便了，逻辑复用更方便了非常利于维护！！

2。 更好的类型推导(typescript支持）

vue3 源码用 ts 重写了，vue3 对ts的支持更友好了（ts 可以让代码更加稳定，类型检测！
...
5。 源码用 typescript 重写，有更好的类型推导(类型检测更为严格，更稳定r
...
1. **then()**：用于处理Promise成功后的回调函数，接受两个参数，第一个是成功回调函数，第二个是失败回调函数。无论成功还是失败，它都会返回一个全新的Promise，因此可以继续进行链式调用。

2.  **catch()**：用于处理Promise失败后的回调函数，当Promise的状态变为rejected时执行。这个方法返回一个新的Promise对象。

3.  **all()**：接受一个Promise对象的数组作为参数，全部成功，Promise.all就返回成功，有一个失败，Promise.all就返回失败

4.  **race()**：也是接受一个Promise数组作为参数，但它只需要数组中任意一个Promise有反应（无论是成功还是失败），它就会立即结束，并返回该Promise的结果。

5.  **allSettled()**：同样是接受一个Promise数组作为参数，它会等待所有Promise都完成（无论是成功还是失败），然后返回一个数组，数组中的每一项都包含对应Promise的状态（fulfilled或rejected）以及结果或原因

6.  **finally()**：当promise得到状态（不论成功或失败）后就会执行finally，。

7.  ##  Promise.any

    any，它也是接受一个promise数组，只要有一个成功，就会执行then，只有当所有的promise都失败，才算失败，此时`Promise.any` 会把所有的失败/错误集合在一起，返回一个失败的 `promise `和[`AggregateError`](https://link.juejin.cn/?target=https%3A%2F%2Fdeveloper.mozilla.org%2Fzh-CN%2Fdocs%2FWeb%2FJavaScript%2FReference%2FGlobal_Objects%2FAggregateError)类型的实例。MDN 上说这个方法还处于试验阶段，如果 `node` 或者浏览器版本过低可能无法使用，各位自行测试下。
...
### 19.深copy和浅copy  （JSON.parse(JSON.stringify(obj))的缺点）

浅copy: 只是增加了一个指针指向已存在的内存地址,拷贝的是对象的属性的引用，而不是对象本身

1.  Object.assign()
2.  解构赋值

深copy: 是增加了一个指针并且申请了一个新的内存，使这个增加的指针指向这个新的内存\
一. JSON.parse(JSON.stringify(obj)) 缺点：不能对undefind,function,symbol,正则，NaN、Infinity和-Infinity深copy
...
二. 手写递归方法（for...in）实现深度克隆原理：遍历对象、数组直到里边都是基本数据类型，然后再去复制，就是深度拷贝。
