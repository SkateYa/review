...
区别：
1.存储⼤⼩区别：cookie数据⼤⼩不能超过4k，sessionStorage和localStorage可以达到5M或者更⼤
2.有效时间：localStorage存储持久数据，浏览器关闭后数据不丢失除⾮主动删除数据；
sessionStorage数据在当前浏览器窗⼝关闭后⾃动删除；cookie设置的cookie过期时间之前
⼀直有效，即使窗⼝或浏览器关闭
3.数据与服务器之间的交互⽅式，cookie在浏览器和服务器间来回传递；sessionStorage和localStorage不会⾃动把数据发给服务器，仅在本地保存
...
### 笔试题
```javascript
Object.prototype.__proto__ === null   原型链的尽头是null
Function.prototype.__proto__ === Object.prototype
构造函数自身的__proto__是什么   Function.prototype 
{  myNew.__proto__ === Function.prototype  }
...
// 创建一个新对象,是通过new Object创建出来的，所以 Object.__proto__ === Object.prototype
...
//   typeof Object === function   object也是函数,是由Function构造出来的
console.log(Person.__proto__ === Function.prototype); // true
// 底层规定好的  Function是由Function构造出来的
...
高优先级任务:浏览器的渲染，对用户的响应（click，mounce事件，定时器，输入，输出，数据上的读取），高优先级任务处理完了有空余时间了才会执行低优先级任务(requestIdleCallback)
...
setInterval 无法保证与浏览器每一帧的渲染同步，所以会出现在一帧里多渲染或少渲染，这就会掉帧，对性能，用户都不友好，
requestAnimationFrame是浏览器的高优先级任务，伴随着浏览器的帧渲染每一步都同步执行，而且每一帧必执行 
...
以上代码定义了一个名为Fn的函数，然后将其原型设置为一个新的Array对象。接着，创建一个新的f对象，它的原型链指向Fn.prototype。最后，代码使用console.log()函数输出了两个表达式的结果，分别检查f对象的构造函数是否等于Fn和Array。

由于Fn.prototype被设置为一个新的Array对象，因此f对象的原型链指向的是Array.prototype。因此，f对象的构造函数实际上是Array而不是Fn。因此，第一个console.log()函数的结果为false，而第二个console.log()函数的结果为true。
```
...
`Promise.all`可以将多个`Promise`实例包装成一个新的 Promise 实例。同时，成功和失败的返回值是不同的，成功的时候返回的是**一个结果数组**，而失败的时候则返回**最先被 reject 失败状态的值**。如果返回中有一个错误就会执行.catch。
...
### 15.微任务(Microtasks)、宏任务(task)是什么？
...
聊聊你知道的设计模式？
<https://juejin.cn/post/6902336983436099597>
