...
- $route 是“路由信息对象”，它是一个局部的对象，每个路由都会有一个对应的route对象。它包含了当前路由的详细信息，如路径、参数、查询参数等（ path，params，hash，query，fullPath，matched，name 等路由信息参数）

- $router router对象则是VueRouter的一个实例，它代表了整个路由系统，包含了所有的路由。router对象提供了路由的跳转方法、钩子函数等，用于控制路由的跳转，对象包括了路由的跳转方法，钩子函数等。new vue的参数

总的来说，route主要用于获取当前路由的信息，而router则主要用于操作路由，如进行路由的跳转、监听等。
