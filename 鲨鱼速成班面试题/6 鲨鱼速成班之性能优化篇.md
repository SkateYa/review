...
- 将元素先设置`display: none`，操作结束后再把它显示出来。因为在display属性为none的元素上进行的DOM操作不会引发回流和重绘。这个过程称为离线操作
...
compression-webpack-plugin版本过高可能会报错
...
productionGzip: true,
```
3. vue.config.js  configureWebpack/plugins下配置
```javascript
new CompressionPlugin({
    test: /\.(js|css|html)?$/i, // 压缩文件格式
    filename: '[path].gz[query]', // 压缩后的文件名,多个文件压缩就有多个.gz文件，建议使用下方的写法
...
5. 这里配置完以后，暂时还不能使用，还需要后端做一下配置，这里后端以nginx为例,nginx实现（nginx.conf）
...
```
6. 响应头多了Content-Encoding: gzip
另外，如果开启gzip压缩，在http请求中，也可以看到响应头多了Content-Encoding: gzip，没开启gzip压缩是没这个的。
